...
plt.stackplot(data_perc.index,
	          ...
	          data_perc['Procedure'], data_perc['Social Issues'], data_perc['Tribute'],
	          labels=list(data_perc),
	          colors =['tab:blue', 'tab:orange', 'tab:green', 'tab:red',
	          'tab:purple', 'tab:brown', 'tab:pink', 'tab:grey', 'tab:olive',
	          'tab:cyan', 'lightgreen'])
plt.legend(loc='upper left', prop={'size': 24})
plt.margins(0,0)
plt.xticks(np.arange(1900, 2014, 4), fontsize=20, rotation=90)
plt.yticks(fontsize=20)
plt.savefig(wd_results + '/figA17.png', dpi=100)
