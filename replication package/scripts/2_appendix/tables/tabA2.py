...
data = joblib.load(wd_aux + '/synth_speeches_tabA2.pkl')
...
def create_sentence_sets(data_a, data_c, pct=pct, n_sent_f=10):
...
os.chdir(wd_results)
with open('tabA2_affect.txt', 'w') as f:
    for item in final_A:
        f.write("%s\n\n" % item)

with open('tabA2_logic.txt', 'w') as f:
    for item in final_C:
        f.write("%s\n\n" % item)
